...
class DMCI(CompressionModel):
    ...
    def forward_train(self, x, qp):
        """
        [TRAINING MODE] Tính toán luồng forward để tính BPP và tái tạo ảnh (Base Frame).
        Bỏ qua khâu nén ra byte (C++) để giữ lại Gradient cho Backpropagation.
        """
        curr_q_enc = self.q_scale_enc[qp:qp+1, :, :, :]
        curr_q_dec = self.q_scale_dec[qp:qp+1, :, :, :]

        # 1. Nén ảnh thành Đặc trưng không gian Y
        y = self.enc(x, curr_q_enc)
        y_pad = self.pad_for_y(y)
        
        # 2. Nén Đặc trưng Y thành Z (Hyper-prior)
        z = self.hyper_enc(y_pad)
        
        # Lượng tử hóa Z (Thêm nhiễu Uniform Noise nếu requires_grad=True)
        z_hat, _ = round_and_to_int8(z)
        
        # Tính Số Bit của Z
        rate_z = self.bit_estimator_z.forward_rate(z_hat, qp)

        # 3. Chạy qua Context Model
        params = self.hyper_dec(z_hat)
        params = self.y_prior_fusion(params)
        _, _, yH, yW = y.shape
        params = params[:, :, :yH, :yW].contiguous()
        ...
        rate_y2 = self.gaussian_encoder.forward_rate(y_q_w_2, s_w_2)
        rate_y3 = self.gaussian_encoder.forward_rate(y_q_w_3, s_w_3)
        rate_y = rate_y0 + rate_y1 + rate_y2 + rate_y3

        # 4. Giải nén thành Ảnh Mờ (Base Frame)
        x_hat = self.dec(y_hat, curr_q_dec).clamp_(0, 1)

        # 5. Tổng hợp Số Bit trên mỗi Điểm ảnh (Bits Per Pixel)
